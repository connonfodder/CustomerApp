...
public class CompanyListModel implements CompanyListContract.Model {
    ...
    @Override
    public Observable<List<Company>> loadCompanyData(CompanyRequest request) {
        HashMap<String, Object> body = new HashMap<>();
        if (Constant.TEST_MODEL) {
            body.put("lng", 103.8506940000);
            body.put("lat", 1.2915100000);
        } else {
            PreferenceUtil preferenceUtil = new PreferenceUtil(CustomerApplication.getInstance());
            body.put("lng", preferenceUtil.getLastLocationLNG());
            body.put("lat", preferenceUtil.getLastLocationLAT());
        }
        body.put("page", request.getPage());
        body.put("restaurantType", request.getRestaurantType() == 0 ? 7 : request.getRestaurantType());
        body.put("foodstylesList", request.getFoodStyles());
        body.put("arrange", Constant.MAX_DISTANCE);
        return API.getDefault().query(body)
    ...
    private List<Company> getData() {
        ...
        data.add(new Company("http://p1.meituan.net/350.214/deal/__24848989__8456253.jpg.webp", "板一回转寿司（龙华店）", 36, Arrays.asList("red", "green", "white"), 123, 7, false));
        ...
        data.add(new Company("http://p1.meituan.net/350.214/deal/c190c2576f46b97a7d8e60378799d411101457.jpg.webp", "佳客来牛排专家（南岭店）", 36, Arrays.asList("chi", "dong", "xi"), 264, 3, false));
