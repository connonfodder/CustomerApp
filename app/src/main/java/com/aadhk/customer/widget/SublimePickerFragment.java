...
public class SublimePickerFragment extends DialogFragment {
	...
	public Pair<Boolean, SublimeOptions> getOptions(Calendar cal) {
		if (cal == null) {
		...
		options.setTimeParams(hour24, minute, true);
	...
	public Pair<Boolean, SublimeOptions> getDateOptions(Calendar cal) {
		if (cal == null) {
			cal = Calendar.getInstance();
		}
		int year = cal.get(Calendar.YEAR);
		...
		SublimeOptions options = new SublimeOptions();
		options.setDateParams(year, month, day);
		options.setTimeParams(hour24, minute, false);
		int displayOptions = 0;
		displayOptions |= SublimeOptions.ACTIVATE_DATE_PICKER;
		options.setDisplayOptions(displayOptions);
		options.setPickerToShow(SublimeOptions.Picker.DATE_PICKER);
	...
	public Pair<Boolean, SublimeOptions> getTimeOptions(Calendar cal) {
		if (cal == null) {
			cal = Calendar.getInstance();
		}
		int year = cal.get(Calendar.YEAR);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		mSublimePicker = (SublimePicker) getActivity().getLayoutInflater().inflate(com.appeaser.sublimepickerlibrary.R.layout.sublime_picker, container);

		// Retrieve SublimeOptions
		Bundle arguments = getArguments();
		SublimeOptions options = null;

		// Options can be null, in which case, pic_default
		// options are used.
		if (arguments != null) {
